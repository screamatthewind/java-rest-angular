...
import com.screamatthewind.auth.PasswordService;
import com.screamatthewind.core.Location;
import com.screamatthewind.core.Token;
import com.screamatthewind.core.User;
import com.screamatthewind.core.User.Provider;
import com.screamatthewind.db.UserDAO;
import com.screamatthewind.utility.ExceptionHandler;
import com.screamatthewind.utility.VerifyReCaptcha;
...
public class AuthResource {
  ...
  public AuthResource(final Client client, final UserDAO dao, final ClientSecretsConfiguration secrets) {
    this.client = client;
    this.dao = dao;
  ...
  public Response login(@Valid final User user, @Context final HttpServletRequest request)
	  ...
	  try {
		  	...
		  	boolean isCaptchaValid = VerifyReCaptcha.verify(user.getCaptchaResponse());
		  	if (!isCaptchaValid)
		  	    return Response.status(Status.UNAUTHORIZED).entity(new ErrorMessage("Invalid Captcha")).build();
		  
		    final Optional<User> foundUser = dao.findByEmail(user.getEmail());
		    if (isCaptchaValid && foundUser.isPresent() && PasswordService.checkPassword(user.getPassword(), foundUser.get().getPassword())) {
		...
		return Response.status(Status.UNAUTHORIZED).entity(new ErrorMessage(LOGING_ERROR_MSG)).build();
			...
			return ExceptionHandler.handleException("login:", user.toString(), e, AuthResource.class);
	}
  }
  ...
  public Response signup(@Valid final User user, @Context final HttpServletRequest request)
	  ...
	  try {
	  	...
	  	User userExists = dao.getUserByEmail(user.getEmail().toUpperCase());		
	    ...
	    return Response.status(Status.CREATED).entity(token).build();
		 	...
		 	return ExceptionHandler.handleException("signup:", user.toString(), e, AuthResource.class);
	  }
  }
